...
import { debaterAgents } from "../lib/agents";

export const saveArtifactTool = createTool({
  id: "saveArtifact",
  description:
    "Saves the final summary and the entire debate log to local files.",
  inputSchema: z.object({
    ...
    inShort: z
      .string()
      .describe("A very concise, 1-2 sentence summary of the entire debate."),
  }),
  execute: async ({ context }: any) => {
    ...
    try {
      ...
      if (activeDebate.currentSelection && activeDebate.allAvailableDebaters) {
        participantsList = activeDebate.currentSelection
          .map((sel) => {
            const debater = activeDebate?.allAvailableDebaters.find(
              (d) => d.id === sel.id,
            );
      ...
      const firstModeratorMsg =
        activeDebate.history.find(
          (msg) => msg.name === "🦸🏻‍♂️ Moderatore" || msg.role === "assistant",
        )?.content || "";
      ...
      if (activeDebate.startTime && activeDebate.endTime) {
        const diffMs =
          activeDebate.endTime.getTime() - activeDebate.startTime.getTime();
...
- **Total Debaters**: ${debaterAgents.length}
